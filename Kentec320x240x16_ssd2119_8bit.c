...
//    GPIOPinTypeGPIOOutput(LCD_RST_BASE, LCD_RST_PIN);LCD_BACKLIGHT_BASE
    ...
    GPIOPinWrite(LCD_DATAH_BASE, LCD_DATAH_PINS, 0x00);    // Here is the problem!!!
...
Kentec320x240x16_SSD2119Init(void)
{
	...
	ROM_SysCtlPeripheralEnable(LCD_DATAH_PERIPH);
    ...
    ROM_SysCtlPeripheralEnable(LCD_CS_PERIPH);
    ROM_SysCtlPeripheralEnable(LCD_BACKLIGHT_PERIPH);
    ...
    // Delay 30mS
    //
    ROM_SysCtlDelay(30 * ulClockMS);
    //ROM_SysCtlDelay(ROM_SysCtlClockGet() / (4 * 30));
